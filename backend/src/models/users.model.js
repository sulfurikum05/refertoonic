...
class UsersModel {
        ...
        static async getVideos () {
          return await pg("videos").select("*").whereIn("id", [2,113,121,122,123,124,125,126,127,128]);
        }
        ...
        static async resetUsersByAdminId(adminId, data, trx) {
          return await pg("users").where({admin_id: adminId}).update(data).transacting(trx)
         }
        static async resetPackage(userId, data, trx) {
          return await pg("users").update(data).where({id: userId}).transacting(trx)
         }
         static async upgradeUsersByAdminId(userId, vipProData, trx) {
          return await pg("users").update(vipProData).where({id: userId}).transacting(trx)
         }
