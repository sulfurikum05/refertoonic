...
export class VipModel {
  ...
  static async getVideos(limit, offset, trx) {
    return await pg("videos")
      .select("id", "video_url", "gif_url", "title", "keywords")
      .where("status", 1)
  ...
  static async getVideosFromWishlist(array, trx) {
    return await pg("wishlist")
      .select("*")
      .whereIn(
        "video_id",
        array.map((item) => item.video_id)
      )
      .whereIn(
        "user_id",
  ...
  static async getNotificationsData(trx) {
    return await pg("notifications")
      .select("*")
      .where({ reciever_vip: "1", sender: "superadmin" })
      .transacting(trx);
  }
  ...
  static async getSenderAdminNotificationsData(adminId, trx) {
    return await pg("notifications")
      .select("*")
      .where({ reciever_vip: "1", sender: adminId })
      .transacting(trx);
  }
}
