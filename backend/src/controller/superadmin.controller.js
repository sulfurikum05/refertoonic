...
export class SuperadminController {
  ...
  static async updateText (req, res, next) {
    try {
      ...
      if (role !== "superadmin") {
        ...
        const data = await SuperadminService.updateText(header, title, text, id)
        SuccessHandlerUtil.handleList(res, next, data)
     }
    } catch (error) {
      next(error)
  ...
  static async updatePaymentPackage (req, res, next) {
    try {
      ...
      if (role !== "superadmin") {
        ...
        const text = req.body.updatedData[2].text || ""
        const id = req.body.id
        const data = await SuperadminService.updatePaymentPackage(title, price, text, id)
      SuccessHandlerUtil.handleList(res, next, data)
     }
    } catch (error) {
      next(error)
  ...
  static async uploadLibraryVideo (req, res, next) {
    try {
      ...
      const data = await SuperadminService.uploadLibraryVideo(videoPath, gifPath, title, keywords, userId)
      SuccessHandlerUtil.handleList(res, next, data)
  ...
  static async bulkUploadLibraryVideo(req, res, next) {
    try {
      ...
      const userId = req.userId;
      const { uploadDir } = req.body;
      const { videos, gifs } = req.files; // Пример, если передаются файлы видео и гифы
  
      // Проверка на наличие видео и гифов
      ...
      // Передаем файлы и другие параметры в сервис
      const data = await SuperadminService.bulkUploadLibraryVideo(videos, gifs, uploadDir, userId);
  ...
  static async sendNotification (req, res, next) {
    try {
      ...
      const data = await SuperadminService.sendNotification(title, message, user, vip, admin, status)
      SuccessHandlerUtil.handleList(res, next, data)
     }
    } catch (error) {
      next(error)
