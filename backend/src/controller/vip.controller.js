 ...
 static async uploadLibraryVideo (req, res, next) {
    ...
    try {
      ...
      if (role == "vip" || role == "admin"){
        ...
        const data = await VipServices.uploadLibraryVideo(videoPath, title, keywords, userId)
        SuccessHandlerUtil.handleList(res, next, data)
      }else{
        return res.status(401).json({ message: 'Unauthorized' });
      }
  ...
  static async upgrade (req, res, next) {
    try {
      ...
      const period = req.body.period
        ...
        const data = await VipServices.upgrade(packageForUpgrade, period, userId, pp)
        SuccessHandlerUtil.handleList(res, next, data)
