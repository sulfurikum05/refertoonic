...
static async register (req, res, next) {
  ...
  static async changePassword (req, res, next) {
    try {
      ...
      const userId = req.userId
      const data = await UsersServices.changePassword(oldPassword, newPassword, newPasswordConfirm, userId)
      SuccessHandlerUtil.handleList(res, next, data)
    } catch (error) {
      next(error)
    }
  ...
  static async sendHelpMessage (req, res, next) {
    try {
      ...
      const data = await UsersServices.sendHelpMessage(userId, userName, userEmail, subject, message)
      SuccessHandlerUtil.handleList(res, next, data)
    } catch (error) {
      next(error)
    }
  ...
  static async upgrade (req, res, next) {
    try {
      ...
      const period = req.body.period
        ...
        const data = await UsersServices.upgrade(packageForUpgrade, period, userId)
        SuccessHandlerUtil.handleList(res, next, data)
