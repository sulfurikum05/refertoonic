...
export class AdminServices {
  ...
  static async createVipProUser(userId, email) {
    ...
    if (usersCountAll <= currentUsersCount) {
      await trx.commit()
      return {message: "Вы достигли лимита на создание пользователей. Для увеличения лимита приобретите доп. пакет."}
    } else{
      ...
      if (user.length !== 0) {
        if (user[0].payment_package == "vipPro") {
          ...
          await AdminModel.updateVipUserPaymentPackageToVipPro(data, email, trx)
              }else if(user[0].role == "user"){
                const data = {role: "vip", payment_package: "vipPro", admin_id: userId}
                await AdminModel.updateUserPaymentPackageToVipPro(data, email, trx)
                }else if(user[0].role == "admin" || user[0].role == "superadmin"){
                  await trx.commit()
                  return {message: "Данного пользователя невозможно добавить в организацию"}
                }
      }else{
